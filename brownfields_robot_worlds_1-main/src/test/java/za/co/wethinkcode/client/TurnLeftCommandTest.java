...
class TurnLeftCommandTest {
    ...
    @BeforeEach
    void setUp() {
        // Initialize with default world config
    ...
    void executeAtWorldBoundary() {
        ...
        robot = new Robot("testBot", new Position(1, 1));
        command = new TurnLeftCommand(world, robot);
        robot.setDirection(Direction.NORTH);
        String response = command.execute();
        assertEquals(Direction.WEST, robot.getDirection());
        verifyResponse(response, Direction.WEST, new Position(1, 1));
    }
    ...
    void executeWithMultipleTurns() {
        // Execute turn left multiple times to ensure consistent behavior
        robot.setDirection(Direction.NORTH);
        String response = command.execute();
        assertEquals(Direction.WEST, robot.getDirection());
    ...
    void executeAtRandomPosition() {
        ...
        String response = command.execute();
        assertEquals(Direction.EAST, robot.getDirection());
        verifyResponse(response, Direction.EAST, new Position(5, 5));
    }
